...
# -----------------------------------------------------------------------
    LAN=$(printf '%s\n' "$1" | tr '[:lower:]' '[:upper:]')
    echo -n "docx..."
    pandoc --reference-doc ${PANDOC_DOCX} \
...
# check if we do have a pandoc
if ! command_exists pandoc; then
    echo "ERR  : Pandoc isn't installed/available on this system..."
    exit 1;
fi

# build PDF doc's
echo "INFO : Remove old reports (html,docx)"
rm -f ${SCRIPT_DOC}/CPU-Report_*.html
rm -f ${SCRIPT_DOC}/CPU-Report_*.docx
echo "INFO : Will use local pandoc installation"
echo "WARN : PDF will not be created using local pandoc $1"
cd ${SCRIPT_DOC}
for i in de en; do
    generate $(basename $i)
done
